...
route.post('/:ul(ru)?/website/edit/:id', preWebsite, requireLogin, function(req, res) {
	...
	Data.access.website(filter)
		.then(function(website) {
			...
			var websiteData = _.pick(req.body, res.locals.userIsAdmin ? ['title', 'description', 'keywords', 'categories', 'contentType', 'country', 'lang', 'status', 'category', 'subcategory'] : ['title', 'description', 'keywords', 'categories', 'contentType', 'category', 'subcategory']);
			websiteData.id = website.id;
			res.locals.website = websiteData;

			if (!websiteData.categories) {
				if (websiteData.category && utils.isNumber(websiteData.category)) {
					websiteData.categories = 'c0-' + websiteData.category;
					if (websiteData.subcategory && utils.isNumber(websiteData.subcategory)) {
						websiteData.categories += ';c1-' + websiteData.subcategory;
					}
				}
				delete websiteData.category;
				delete websiteData.subcategory;
			}
